<template>
  <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-position="left" label-width="0px" class="demo-ruleForm login-container">
    <h3 class="title">云教育系统学生端登录入口</h3>
  ...
  export default {
    data() {
         var validateName = (rule, value, callback) => {
            var reg=/^[0-9]{0,11}$/;
            if (value==="") {
                 return callback(new Error('请输入您的学号'));
             }
             if(!reg.test(value)){
                    return callback(new Error('学号格式不正确，应为全数字'));
             }else{
                 callback();
             }
         };
      ...
      handleSubmit2(ev) {
        ...
        this.$refs.ruleForm2.validate((valid) => {
          if (valid) {
            ...
            this.logining = true;
            //NProgress.start();
            var loginParams = { username: this.ruleForm2.account, password: this.ruleForm2.checkPass,rememberMe:this.checked};
