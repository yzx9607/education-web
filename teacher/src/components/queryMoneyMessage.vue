<template>
		<!--审核部分的缴费详情(根据id)(my_322.vue里的详情的弹出层)-->
		<el-dialog title="缴费详情" v-model="cardDetailVisible" :close-on-click-modal="false">
...
<script>
	import { getCheckQueryMoneyMessage } from '../api/api';
	export default {
		data() {
			return {
		...
		methods: {
			...
			showCardDetail(row) {
				...
				getCheckQueryMoneyMessage(para).then((res) => {
